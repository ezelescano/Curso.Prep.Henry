...
function asAmirror(str) {
  ...
  for(var i = 0 ; i < stringArray.length ; i++) {
      var palabraEspejo = stringArray[i].split('').reverse().join('');   
      arrayMirror.push(palabraEspejo);
      var nuevaString = arrayMirror.join(' ');
    }
  return(nuevaString);
}
...
function capicua(numero){
  ...
  var nuevaString = numString.split('').reverse().join('');
  if (numString === nuevaString){
    return('Es capicua');
  ...
  var arrayCadena = cadena.split('');
    for (var i = 0 ; i < arrayCadena.length ; i++) {
        if (arrayCadena[i] !== 'a' && arrayCadena[i] !== 'b' && arrayCadena[i] !== 'c') {          
          stringComparado += arrayCadena[i];         
        } 
    } return(stringComparado);
}
